...
class Empresa(models.Model):
    ...
    art_contratada = models.OneToOneField(
        'Art', on_delete=models.CASCADE, related_name='aseguradora')
...
class Recibo(models.Model):
    ...
    extra = models.ForeignKey(
        Extra, blank=True, null=True, on_delete=models.CASCADE, default=0)
    legajo_empleado = models.OneToOneField(Empleado, on_delete=models.CASCADE)

    def save(self, *args, **kwargs):
        monto_deduccion = self.montoBruto * \
            (self.deduccion.porcentaje_deduccion / 100)
        if self.extra is None:
            self.montoNeto = self.montoBruto + self.asistencia - monto_deduccion
        else:
            self.montoNeto = self.montoBruto + self.extra.monto_extra + \
                self.asistencia - monto_deduccion
...
class Reclamo(models.Model):
    ...
    recibo = models.OneToOneField(
        Recibo, blank=True, null=True, on_delete=models.CASCADE)
...
class Factura(models.Model):
    ...
    Pedido = models.OneToOneField(
        Pedido, blank=True, null=True, on_delete=models.CASCADE)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.Pedido:
            self.valorFactura = (self.Pedido.valorTotal *
                                 self.IVA) + self.Pedido.valorTotal
        super().save(*args, **kwargs)
